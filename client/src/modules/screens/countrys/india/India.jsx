...
const India = () => {
  const content = lang[0].india;

  const Section = ({ titulo, descripcion, lista, isSingleColumn }) => (
    <div className="mb-6">
      <h2 className="text-4xl sm:text-5xl lg:text-3xl xl:text-5xl text-center text-TVBlue font-bold py-6">
        {titulo}
      </h2>
      {descripcion && (
    ...
    <main className="w-full h-full py-10 bg-TVgray">
      <div className="w-[95%] md:w-[85%] m-auto mb-10">
        ...
        {content.first_section.secciones.map((section, index) => (
          <Section key={index} {...section} isSingleColumn={true} />
        ))}
        ...
        <footer className="py-4">
          <h2 className="text-2xl sm:text-3xl lg:text-2xl xl:text-3xl text-center text-TVBlue font-bold py-6">
            Fuentes
          </h2>
          <ul className="list-inside text-justify lg:text-md xl:text-lg">
            {content.footer.fuentes.map((fuente, index) =>
              typeof fuente === "string" ? (
                <li key={index} className="pb-2">
                  <strong>-</strong> {fuente}
                </li>
              ) : (
                <li key={index} className="pb-2">
                  <strong>-</strong>{" "}
                  <a
                    href={fuente.url}
                    className="text-TVBlue hover:cursor-pointer underline"
                    target="_blank"
                  >
                    {fuente.texto}
                  </a>
